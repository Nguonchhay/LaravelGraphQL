...
cd laradock
git checkout 61d7b18922006d582ae68ca0cdf5e202e5bcf08d
#git checkout v6.0.1
cp ./../local_setup/env-example .env
rm ./../laradock/mysql/Dockerfile
rm ./../laradock/mysql/my.cnf
cp ./../local_setup/mysql/Dockerfile ./../laradock/mysql
...
if [ $? -eq 0 ]; then
  ...
  docker-compose up -d nginx mysql phpmyadmin mailhog || exit -1
fi
...
hostname="local.feited.host test-local.feited.host"
